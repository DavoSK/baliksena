...
DataFormat4DS::Morph DataFormat4DS::loadMorph(std::ifstream& file,
                                              bool ignoreStandard) {
    Morph newMorph = {};
    ...
    if (newMorph.mFrameCount) {
        ...
        for (size_t i = 0; i < newMorph.mLODLevel; ++i) {
            ...
            for (size_t j = 0;
                 j < newMorph.mFrameCount * newMorphLod.mVertexCount; ++j) {
                MorphLodVertex v;
                read(file, &v, sizeof(v));
...
void DataFormat4DS::loadMesh(Model* model, std::ifstream& file) {
    ...
    for (size_t i = 0; i < model->mMeshCount; ++i) {
        ...
        std::string meshParams = std::string(newMesh.mMeshParams);

        if (meshName.find("wcol") != meshName.npos) {
            newMesh.mMeshType = MESHTYPE_COLLISION;
        }

        if(meshParams.find("OFF_") != meshParams.npos ||
            meshParams.find("off_") != meshParams.npos) {
            newMesh.mMeshType = MESHTYPE_COLLISION;
        }
