...
{
public:
    virtual bool load(std::istream &srcFile) override; ///< Loads the file table from the executable file.
    typedef struct
    {
...
private:
    ...
    std::vector<DTAFile> fetchExecutableKeys(std::istream &srcFile, std::vector<DTAFile> & filesToFetch, uint32_t startOffset);
};
